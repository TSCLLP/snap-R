...
export function StudioClient({ listingId, userRole, showMlsFeatures = false, credits = 0 }: { listingId: string; userRole?: string; showMlsFeatures?: boolean; credits?: number }) {
  ...
  const handleEnhance = async (toolId: string) => {
    ...
    trackEvent(SnapREvents.ENHANCEMENT_STARTED, { tool: toolId });
    ...
    try {
      ...
      if (data.success && data.enhancedUrl) {
        trackEvent(SnapREvents.ENHANCEMENT_COMPLETED, { tool: toolId });
            ...
            <>
                ...
                {pendingEnhancement ? (
                  ...
                  >
                    <img 
                      ...
                      style={{ filter: getFilterStyle(), opacity: adjustments.intensity / 100 }}
                      draggable={false}
                    />
                    <div 
              ...
              {pendingEnhancement && (
                <AdjustmentPanel
                  ...
                  onDiscard={() => { setPendingEnhancement(null); setAdjustments({ intensity: 100, brightness: 0, contrast: 0, saturation: 0, warmth: 0 }); }}
